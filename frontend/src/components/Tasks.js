...
const Tasks = ({
  ...
  const submitEdit = async ({ type, payload }) => {
    ...
    try {
      const res = await axios.put(`http://localhost:3000/api/v1/tasks`, edit[type](payload))
  ...
  const deleteTask = async (id) => {
    apiRequest('deleting')
    try {
      const res = await axios.delete(`http://localhost:3000/api/v1/tasks/${ id === 'all' ? 'deleteall' : id }`)
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="w-full max-w-xs mt-16">
        <AddNew input={ tasks.userInput } addTask={ addTask } handleUserInput={ handleUserInput } inputEmpty={ tasks.inputEmpty } />
      </div>
      {
        loading ? 
        <p>loading...</p>
